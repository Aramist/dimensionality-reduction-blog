...
def make_mds_embedding(points):
    return MDS(normalized_stress='auto').fit_transform(points)
...
def add_noise_like(arr: np.ndarray, snr_db: float = 20.0):
    """Adds noise to an array of points
    ...
    """
    # snr = 20 * log10(ratio)
    # inverted: ratio = 10**(snr / 20)
    noise_ratio = 10 ** (snr_db / 20)  # ratio of signal / noise
    noise = np.random.randn(*arr.shape)
    noise /= np.linalg.norm(noise, axis=0, keepdims=True)  # Normalize across features
    sig_norm = np.linalg.norm(arr, axis=0, keepdims=True)
    noise *= sig_norm / noise_ratio
    return arr + noise
...
def sample_sphere(manifold_dim, num_points, snr=None):    
    ...
    if snr is not None:
        lowd_points = add_noise_like(rand_vecs, snr)
    else:
        lowd_points = rand_vecs
    return lowd_points


def sample_disk(num_points, snr=None):
    angles = np.random.uniform(0, 2*np.pi, size=(num_points,))
    radii = np.random.uniform(0, 1, size=(num_points,)) ** 0.5
    disk_dataset_lowd = np.stack([np.cos(angles), np.sin(angles)], axis=1) * radii[:, None]
    if snr is not None:
        disk_dataset_lowd = add_noise_like(disk_dataset_lowd, snr)
    return disk_dataset_lowd


def sample_cube(manifold_dim, num_points, snr=None):
    ...
    lowd_points = rand_vecs
    if snr is not None:
        lowd_points = add_noise_like(lowd_points, snr)

    return lowd_points

def sample_torus(num_points, tube_radius, outer_radius, snr=None):
    ...
    if snr is not None:
        points = add_noise_like(points, snr)
    return points
